...
- *USN Journal*: The Update Sequence Number (USN) Journal is a log maintained by NTFS to record changes made to files and directories. Forensic investigators can analyze the USN Journal to track file modifications, deletions, and renames. It is called the change journal, and it is present in the `$Extend` record in the [[File System#MFT - Master File Table|MFT]].
