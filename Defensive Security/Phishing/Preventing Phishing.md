...
- SPF records are [[Domain Name System (DNS)]] *TXT* records that contain a list of the IP addresses that are allowed to send emails on behalf of the domain.
...
When an email is sent, the sending mail server uses a private key to add a [[Digital Signatures]]. The signature is added to the email in the `DKIM-Signature header`.
