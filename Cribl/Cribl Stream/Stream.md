...
Cribl Stream provides users to route data to any destination.
- Can obtain data from any source and send to any destination
- Can also do some processing in Cribl stream to do any changes.
...
- This is a group of machines that work together to process data..
- Worker groups are logical groups for the worker nodes.
- These groups can be sepearated by location or by data type and by function.
- worker groups can have as many worker nodes.

A leader node manages all the configs for the worker and edge nodes, and sends the config information.
- Comm takes place by sending configs on how to collect data based on rules.
- The leader communicates via TCP [[Port]] `4200`.
- Also uses [[HTTP]] port 4200 to download bundles.
...
- Create a `cribl` user to own all files and runs all processes.
...
Gives admins the ability to provide access to data.
- Creates spaces for teams/users to access data.
- Access to sources/dest/pipelines/more.
- Projects are based on worker groups.

**Project Editors**:
- Can adjust the data flow for the entire project.
- Can configure other objects and commit changes to git.

**Cribl Admin**:
- Can create subscriptions/projects.
- Provides users with access to different projects

**Subscriptions**
- To specify a subset of the workers group data.
- Multiple subscriptions can be assigned to a project.

**Projects**
- Connections between subscriptions and destinations.
- Assign multiple subscriptions with multiple destinations
