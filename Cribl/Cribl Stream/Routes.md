...
- **QuickConnect**: Fast and simple drag and drop UI to connect sources with destinations. Can use [[Pipelines, Functions, Packs#Packs|Packs]] here.
...
These are basically the expressions that evaluate to true or false and help in routing.
- There are many functions present.

> The Cribl Expressions Methods all start with a `C.` and these are native custom functions that can be used to assist in filtering.
...
Can also add dynamic destinations when creating the routes by toggling the `Enable Expression` toggle.
- Then, can write a JavaScript expression that is evaluated to a destination.
- This is useful to send data to multiple locations in 1 route.
...
A route can be unreachable if the previous route:
- Is enabled
- Is set to Final
- It's filter expression evaluates to True.

---
### Cloning & Good Practice

If cloning will not be used, then have the routes in the beginning to have broad expressions.

If cloning will be used on a narrow set of events, then do that at the beginning and route them quickly to consume them.
- Data that gets cloned most likely gets sent to object storage, so its best to put these routes at the top.
